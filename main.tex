...
\hspace{1cm}\includegraphics[height=.13\linewidth]{figs/logo_telecom_paris_ipp.png}}
...
$\bullet$ \textbf{Forward process:} define a noise schedule ${\beta_t\in(0,1)}$,
...
$\bullet$ \textbf{Backward process:}  and enables sampling:
  \[
    \zz_{t-1}=
    \frac{1}{\sqrt{\alpha_t}}
...
\headerbox{3) DMs Conditioning}{name=dm_conditioning,column=1,span=2}{
  \begin{center}
    \textbf{Training}
    
  \begin{minipage}{.45\linewidth} \centering
      \includegraphics[width=\linewidth]{figs/cldm_train-Page-1.drawio.pdf}\par
      \vspace{0.2em}
      {\small \textbf{ControlNet style conditioning (CN)} \\ SD UNet is frozen}\par
   \end{minipage}
   \begin{minipage}{.54\linewidth} \centering
     \vskip 3mm
     \includegraphics[width=\linewidth]{figs/ldm_train-Page-1.drawio.pdf}\par
     \vspace{0.2em}
   ...
   $
     ...
     \left[ \,\big\|\boldsymbol{\epsilon}_t - \boldsymbol{\epsilon}_\theta(\zz_t, t,\zz_{LQ},b)\big\|_2^2 \,\right] .
   $
  ...
  \begin{minipage}{.53\linewidth} \centering
    ...
    \includegraphics[width=\linewidth]{figs/ldm_sample-Page-2.drawio.pdf} \\
  \end{minipage}
  \begin{minipage}{.46\linewidth}
  %   \begin{center}
  %     \textbf{Controllable Blind Deblurring} \\[0.1em]
  %     \includegraphics[width=\linewidth]{figs/BxU_diagram.drawio.pdf}
  %   \end{center}[-0.9em]
  %   \small
  ...
  \includegraphics[width=.9\linewidth]{figs/BxU_diagram.drawio.pdf}\\[-0.9em] 
  \scriptsize
...
\midrule
DiffBIR          & 18.3051 & 0.5284 & 0.2278 & \second{71.6563} & 0.5986 & \second{0.7107} \\
PASD             & \best{21.9344} & \best{0.6518} & 0.2138 & 68.9798 & 0.4850 & 0.5904 \\
SeeSR            & 20.1731 & 0.5808 & 0.2463 & 70.4701 & 0.5529 & 0.6541 \\
ResShift         & 19.7569 & 0.4976 & 0.4138 & 48.0120 & 0.2960 & 0.4199 \\
\midrule
...
{\small LQ} & {\small DiffBIR} & {\small PASD} & {\small SeeSR} & {\small ResShift} & {\small SupS-CN} & {\small SupS-FT} & {\small HQ} \\[0.1em]
...
\headerbox{5) Real-world Results}{name=results_real,column=1,span=2,below=results_synth}{ 
...
\headerbox{6) Conclusion}{name=conclusion,column=0,span=1,below=diffusion}{
  ...
  $\bullet$ The ControlNet approach performs better w.r.t. no-reference metrics but is biased by the frozen SD backbone. \\
  $\bullet$ The fine-tuning approach achieves better perceptual similarity (LPIPS) and is generally more faithful to the blurry input.

 \textsc{Future work}: Enforce data consistency and force identity-behavior when the blur measure is zero, Explore newer generative backbones.
  
}

\headerbox{References}{name=references,column=0,span=1,below=conclusion}{
  \footnotesize
 $\cdot$ R. Rombach et al. %A. Blattmann, D. Lorenz, P. Esser, \& B. Ommer
  (2022). High-resolution image synthesis with latent diffusion models.
  In Proc. of CVPR. \\ % the IEEE/CVF conference on computer vision and pattern recognition (pp. 10684-10695). \\
  $\cdot$ J. Ho, A. Jain, \& P. Abbeel (2020). Denoising diffusion probabilistic models. Advances in neural information processing systems, 33, 6840-6851.\\
 $\cdot$ L. Zhang et al. % A. Rao, \& M. Agrawala
  (2023). Adding conditional control to text-to-image diffusion models.
  In Proc. of ICCV. \\% the IEEE/CVF international conference on computer vision (pp. 3836-3847).
  $\cdot$ Lin et al. %, X., He, J., Chen, Z., Lyu, Z., Dai, B., Yu, F., ... & Dong, C. 
  (2024). DiffBIR: Toward blind image restoration with generative diffusion prior. In European conference on computer vision
